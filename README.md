...
- **[dataproduct-bootstrap](https://github.com/entropy-data/dataproduct-builder-dbt/blob/main/skills/dataproduct-bootstrap/SKILL.md)** scaffolds a new dbt data product from scratch: `dbt_project.yml`, model layout, README with `uv` install instructions, `profiles.yml.example` for the chosen warehouse, then hands off to the sync skill.
- **[dataproduct-implement](https://github.com/entropy-data/dataproduct-builder-dbt/blob/main/skills/dataproduct-implement/SKILL.md)** takes an Entropy Data data product URL or id, fetches its data contracts, translates the ODCS schema into dbt models under `models/output_ports/v1/` (column list, types, tests), and implements the model bodies by mapping declared input ports to output columns. Complex joins are left as TODOs.
- **[dataproduct-exampledata](https://github.com/entropy-data/dataproduct-builder-dbt/blob/main/skills/dataproduct-exampledata/SKILL.md)** extracts ~20 sample rows via a non-prod dbt profile, drops PII columns flagged in the contract (and obvious name-based PII), and uploads the scrubbed sample with `entropy-data example-data put`. Two explicit user confirmations before anything leaves the machine.
